...
对于一个字符串中有空格，可以使用[《字符串(4)》](https://github.com/qiwsir/StarterLearningPython/blob/master/109.md)中提到的`strip()`等。但是，它不是仅仅去掉一个空格，而是把字符串两边的空格都去掉。都去掉似乎也没有什么关系，再用空格把单词拼起来就好了。
